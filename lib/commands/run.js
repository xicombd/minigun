  ...
  async.waterfall([
    ...
    function(err, result) {
      ...
      if (options.environment) {
        if (options.environment in result.script.config.environments) {
          runnerOpts.environment = options.environment;
        } else {
          console.log(
            'Environment "%s" does not seem to be defined in %s',
            options.environment, scriptPath);
        }
      }

      var ee = runner(result.script, result.payload, runnerOpts);
